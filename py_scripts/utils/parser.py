...
class parser:
            ...
            if(item.upper() in data.Tcategory()):
                ...
                if len(strings) == 2:
                    ...
                    if(tmp in data.Tversion()): #check if this is version or dev
                        Titem['version'] = strings[1].replace('[',"").replace(']',"").strip()
                    else:
                        Titem['developer'] = strings[1].replace('[',"").replace(']',"").strip()
                elif len(strings) == 3:
                    Titem['version'] = strings[1].replace('[',"").replace(']',"").strip()
                    Titem['developer'] = strings[2].replace('[',"").replace(']',"").strip()  

                Titem['f95_id'] = re.sub('[\W_]+', '',Titem['site_url'].split(".")[len(Titem['site_url'].split(".")) - 1])
                Titem['short_name'] = re.sub('[\W_]+', '',Titem['title'].strip().replace(" ","")).upper()   
    ...
    def ParseRating(element):
        try:
             return element.select('span.ratingStars')[0].text.strip().replace("star(s)","").strip()
        except:
            return ""
        
    def ParseViews(element):
        try:
             return element.select('div.structItem-cell--meta')[0].find_all('dl')[1].select('dd')[0].text
        except:
            return ""
        
    def ParseReplies(element):
        try:
             return element.select('div.structItem-cell--meta')[0].find_all('dl')[0].select('dd')[0].text
        except:
            return ""
