...
function Stat({ label, value, sub, accent, onClick }) {
  ...
  return (
    <div
      className={`panel stat ${onClick ? 'stat-click' : ''}`}
      onClick={onClick}
        ...
        <>
          <div className="stat-grid">
            ...
            <Stat label="LC review queue" value={data.queue} accent="var(--amber)"
              sub="awaiting a decision" onClick={() => navigate('/admin/queue')} />
          </div>
          ...
          <div className="panel panel-pad" style={{ marginTop: 18 }}>
            <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <button className="btn btn-sm" onClick={() => navigate('/admin/changelog')}>View all</button>
            </div>
            {recent === null
              ? <Spinner label="Loading activity…" />
              : <ChangelogList entries={recent} compact onOpenGame={(id) => navigate(`/admin/atlas?focus=${id}`)} />}
          </div>
        </>
      )}
    </>
